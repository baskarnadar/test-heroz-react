...
const REMOVE_URL = 'https://api.heroz.sa/api/vendordata/activityinfo/activity/removeattachImages'
...
const ImageUploadModal = ({ isOpen, onClose, onSaved, activity }) => {
    ...
    const fetchImages = async () => {
      ...
      try {
        const res = await fetch(
          `${API_BASE_URL}/vendordata/activityinfo/activity/getattachImages`,
          {
            method: 'POST',
  ...
  const attachToActivityIfSupported = async (key) => {
    try {
      const res = await fetch(`${API_BASE_URL}/vendordata/activityinfo/activity/attachImages`, {
        method: 'POST',
        headers: { ...getAuthHeaders(), 'Content-Type': 'application/json' },
        body: JSON.stringify({
